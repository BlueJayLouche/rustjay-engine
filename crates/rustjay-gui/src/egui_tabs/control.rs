...
impl EguiControlGui {
    pub(crate) fn build_midi_tab(&mut self, ui: &mut egui::Ui) {
        ...
        if learn_active {
            ...
            ui.label(
                egui::RichText::new(&waiting_text)
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if let Some(m) = mapping {
                                    let label = match m.kind {
                                        MidiMsgKind::Cc => {
                                            format!("CC {} ch{}", m.selector, m.channel)
                                        }
                                        MidiMsgKind::Note => {
                                            format!("Note {} ch{}", m.selector, m.channel)
                                        }
                                    ...
                                    ui.label(
                                        egui::RichText::new(label)
                                            .size(11.0)
                                            .color(egui::Color32::from_rgb(0, 220, 130)),
                                    );
                                } else {
                                    ui.label(
        ...
        if midi_mappings.is_empty() {
            ...
            for m in &midi_mappings {
                ...
                ui.label(
                    egui::RichText::new(format!("  {} → {}", m.name, binding))
                        .size(11.0)
                        .monospace(),
                );
            }
    ...
    pub(crate) fn build_osc_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            let response = ui.add(
                egui::DragValue::new(&mut port_i32)
                    .speed(1.0)
                    .range(1024..=65535),
            );
            ...
            for cat in &sorted_categories(&descriptors) {
                ...
                egui::CollapsingHeader::new(cat.name())
                    ...
                    .show(ui, |ui| {
                        for desc in &cat_params {
                            let addr =
                                format!("/rustjay/{}/{}", cat.name().to_lowercase(), desc.id);
        ...
        egui::CollapsingHeader::new("Audio")
            ...
            .show(ui, |ui| {
                ui.label(
                    egui::RichText::new("/rustjay/audio/amplitude")
                        .monospace()
                        .size(12.0),
                );
                ui.label(
        ...
        egui::CollapsingHeader::new("Output")
            ...
            .show(ui, |ui| {
                ui.label(
                    egui::RichText::new("/rustjay/output/fullscreen")
                        .monospace()
                        .size(12.0),
                );
                ui.label(
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for (addr, value, _time) in messages.iter().rev().take(20) {
                        ui.label(
                            egui::RichText::new(format!("{} = {:.3}", addr, value))
                                .monospace()
                                .size(12.0),
                        );
                    }
    ...
    pub(crate) fn build_web_tab(&mut self, ui: &mut egui::Ui) {
        ...
        if enabled && !full_url.is_empty() {
            ...
            if let Some((_, matrix)) = &self.qr_cache {
                ...
                let (rect, _) =
                    ui.allocate_exact_size(egui::vec2(qr_size, qr_size), egui::Sense::hover());
        ...
        if lan_trust {
            ui.label(
                egui::RichText::new(
                    "Any device on your local network can connect without a token.",
                )
                .size(11.0)
