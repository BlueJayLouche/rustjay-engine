...
//! the first channel to f32 and pushes into a bounded queue, and the engine
//! thread drains it each frame. The decoder downstream tolerates the odd
//! dropped sample, so the callback never blocks or allocates.
...
/// Names of the available audio input devices on the default host.
pub fn list_input_devices() -> Result<Vec<String>, InputError> {
    cpal::default_host()
        .input_devices()
...
/// Push the first channel of interleaved `data` onto `queue` as f32.
///
...
fn push_mono<S>(queue: &ArrayQueue<f32>, data: &[S], channels: usize)
where
    S: cpal::SizedSample,
    f32: FromSample<S>,
{
...
impl InputCapture {
    /// Open `device_name` (empty = the default input device) and start
    /// capturing immediately.
    pub fn start(device_name: &str) -> Result<Self, InputError> {
        ...
        let stream = match config.sample_format() {
            cpal::SampleFormat::F32 => device.build_input_stream(
                stream_config,
                move |data: &[f32], _| push_mono(&writer, data, channels),
                on_error,
                None,
            ),
            cpal::SampleFormat::I16 => device.build_input_stream(
                stream_config,
                move |data: &[i16], _| push_mono(&writer, data, channels),
                on_error,
                None,
            ),
            cpal::SampleFormat::U16 => device.build_input_stream(
                stream_config,
                move |data: &[u16], _| push_mono(&writer, data, channels),
                on_error,
                None,
            ),
        ...
        log::info!("[audio-in] Capturing '{name}' at {sample_rate} Hz, {channels} ch (ch 1 used)");
        Ok(Self {
            _stream: stream,
            queue,
            device_name: name,
...
mod tests {
    ...
    #[test]
    fn push_mono_converts_formats_and_takes_channel_one() {
        let queue = ArrayQueue::new(16);
        push_mono(&queue, &[0.0f32, 9.0, -1.0, 9.0, 0.5, 9.0], 2);
        assert_eq!(queue.pop(), Some(0.0));
        assert_eq!(queue.pop(), Some(-1.0));
        assert_eq!(queue.pop(), Some(0.5));
        assert!(queue.is_empty());

        push_mono(&queue, &[i16::MAX, 0, i16::MIN, 0], 2);
        assert!((queue.pop().unwrap() - 1.0).abs() < 1e-4);
        assert!((queue.pop().unwrap() - -1.0).abs() < 1e-4);

        push_mono(&queue, &[u16::MAX, 0, u16::MIN, 0, 32768, 0], 2);
        assert!((queue.pop().unwrap() - 1.0).abs() < 1e-4);
        assert!((queue.pop().unwrap() - -1.0).abs() < 1e-4);
    ...
    fn push_mono_drops_on_overflow_instead_of_blocking() {
        ...
        push_mono(&queue, &[1.0f32, 2.0, 3.0], 1);
