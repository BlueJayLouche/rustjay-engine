# Packages cuepool.exe + its FFmpeg DLL closure into a self-contained, shareable folder + zip.
...
$exe  = Join-Path $root 'examples\cuepool\target\release\cuepool.exe'
$obs  = 'C:\Program Files\obs-studio\bin\64bit'                 # FFmpeg 7.x shared build source
$out  = Join-Path $root 'dist\cuepool'

if (-not (Test-Path $exe)) { throw "Build first: cargo run --release --manifest-path examples/cuepool/Cargo.toml -p cuepool  (or build). Missing: $exe" }
...
$zip = Join-Path $root 'dist\cuepool-windows.zip'
