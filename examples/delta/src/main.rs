...
impl AnyGuiTab for MotionTab {
    ...
    fn draw(
        ...
        // Intensity & blend
        if ui.slider_config("Intensity", 0.0, 1.0).build(&mut state.intensity) {
        ...
        if ui.slider_config("Red Gain", -2.0, 2.0).build(&mut state.red_gain) {
            engine.set_param_base("red_gain", state.red_gain);
        }
        if ui.slider_config("Green Gain", -2.0, 2.0).build(&mut state.green_gain) {
            engine.set_param_base("green_gain", state.green_gain);
        }
        if ui.slider_config("Blue Gain", -2.0, 2.0).build(&mut state.blue_gain) {
        ...
        if ui.slider_config("Input Mix", 0.0, 1.0).build(&mut state.input_mix) {
            engine.set_param_base("input_mix", state.input_mix);
        }
        if ui.slider_config("Trail Fade", 0.0, 1.0).build(&mut state.trail_fade) {
            engine.set_param_base("trail_fade", state.trail_fade);
        }
        if ui.slider_config("Threshold", 0.0, 1.0).build(&mut state.threshold) {
            engine.set_param_base("threshold", state.threshold);
        }
        if ui.slider_config("Smoothing", 0.0, 1.0).build(&mut state.smoothing) {
